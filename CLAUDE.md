...
- Блок может быть контейнером для чужого жеста (канбан-зона внутри блока): `gridCore` игнорирует `pointerdown`, пришедший из `[data-flip-id]` (метку ставят и `sortableCore`, и `sortableGroup`), а блоку нужна `[data-drag-handle]`. Блоки рендерятся `<For each={props.items}>`, НЕ по `layout()`: тот пересоздаётся на каждом дропе и пересоздавал бы DOM блоков вместе с вложенными ref'ами. Порядок в DOM не важен — позиция задана явными `grid-column/row`.
