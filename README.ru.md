...
Хвост `&path:/packages/<имя>` выбирает нужный пакет, а `#<тег>` прибивает версию: теги короткие, по имени папки — `table@0.5.0`, `grid-dnd@0.5.0`. Собранный `dist/` лежит в репозитории, поэтому собирать у себя ничего не нужно.

Обновляются пакеты **по отдельности** — ради этого разбивка и делалась: меняешь тег у одного, остальные остаются как были.
