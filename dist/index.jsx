...
// src/Virtual/solid.ts
import { createSignal as createSignal4, onCleanup as onCleanup3 } from "solid-js";

// src/Virtual/virtualMath.ts
function buildOffsets(keys, h) {
...
// src/Virtual/virtualCore.ts
var EMPTY = { first: 0, last: 0, padTop: 0, padBottom: 0, total: 0 };
function createVirtualEngine(opts) {
  const attr = opts.keyAttr ?? "data-key";
  ...
  function recompute() {
    ...
    const changed = next.first !== win.first || next.last !== win.last || next.padTop !== win.padTop || next.padBottom !== win.padBottom;
    win = next;
    if (changed) opts.onChange(win);
  }
  function measure2() {
    if (!scroller || measuring) return;
    const els = Array.from(scroller.querySelectorAll(`[${attr}]`));
...
// src/Virtual/solid.ts
var EMPTY2 = { first: 0, last: 0, padTop: 0, padBottom: 0, total: 0 };
function createVirtual(opts) {
  const [win, setWin] = createSignal4(EMPTY2);
  const engine = createVirtualEngine({ ...opts, onChange: setWin });
  onCleanup3(engine.destroy);
  return {
    /** ref на прокручиваемый контейнер */
    scroller: (el) => onCleanup3(engine.attach(el)),
