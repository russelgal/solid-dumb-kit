...
Таблица не виртуализирует, и это сознательно: самодельное окно означает оценку высот строк, коррекцию `scrollTop`, когда оценка не сошлась, и врущую полосу прокрутки, пока всё не измерено. На строках разной высоты это разваливается быстро.

Пусть работу пропускает браузер — он это умеет:

```css
.row { content-visibility: auto; contain-intrinsic-size: auto 40px }
```

Строки за экраном остаются в DOM (поиск по странице и скринридеры работают), но не раскладываются и не красятся. `contain-intrinsic-size: auto` заставляет браузер запомнить фактический размер уже показанной строки — поэтому **высоты могут быть любыми**, и считать нечего.

Если окно всё-таки нужно (например, данные подтягиваются с сервера порциями), `spacerTop` / `spacerBottom` рисуют пустые строки заданной высоты — свой срез разместишь внутри полной высоты сам. Сортируй тогда снаружи, ровно как при пагинации, и выключи перетаскивание: позиции снимаются один раз, а строк вне окна в DOM нет.
