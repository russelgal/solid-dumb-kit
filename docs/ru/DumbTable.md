...
Таблица не виртуализирует за тебя — та же история, что с пагинацией: окно режешь ты, она рисует, что дали. От неё только `spacerTop` / `spacerBottom` — пиксели, съеденные строками выше и ниже окна, чтобы полоса прокрутки осталась настоящей:
...
Из того, что строк вне окна в DOM нет, следуют две вещи:

- **сортируй снаружи**, ровно как при пагинации — иначе отсортируешь один экран;
- **выключи перетаскивание** (`onReorder` не передавать). Позиции снимаются один раз на старте жеста, а не отрисованных строк в снимке быть не может. У выделения рамкой то же ограничение: оно захватит только то, что было на экране в момент начала жеста.

В примере есть переключатель пагинация/виртуал — можно сравнить оба режима.
