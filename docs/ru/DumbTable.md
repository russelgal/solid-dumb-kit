...
`<table>` — единый layout-контекст: при `table-layout: auto` браузер меряет каждую ячейку, чтобы вычислить ширины колонок, и `content-visibility` тут не спасает — containment не применяется к `table-row`. Поэтому пять тысяч строк раскладываются целиком, и это чувствуется.

Отсюда `createVirtual` — окно над данными плюс `spacerTop` / `spacerBottom`, чтобы полоса прокрутки не врала:
...
createEffect(() => { windowRows(); virt.measure() })   // снять высоты отрисованного
createEffect(() => { rows(); virt.refresh() })         // данные сменились
...
**Высоты строк могут быть любыми.** Отрисованные измеряются одним батчем через `IntersectionObserver` (без reflow и без `getBoundingClientRect` по каждой строке) и запоминаются; для ещё не показанных берётся `estimate`. Прокрутку при уточнении высот мы намеренно **не** подправляем — именно эта коррекция заставляет такие списки ползти под курсором. Окно просто пересчитывается от текущего `scrollTop`.

Отсюда два следствия, те же, что и при пагинации: **сортируй снаружи** и **выключи перетаскивание** — строк за пределами окна в DOM нет, значит и в снимке позиций их быть не может. У выделения рамкой то же ограничение.

Ещё стоит задать `table-layout: fixed` и явные ширины колонок: это избавляет браузер от измерения ячеек ради ширин.
