...
**Известное ограничение.** Даже при правильной разводке выделение — это всегда «то, что рамка накрывает сейчас»: так устроен `@viselect/vanilla` (`_updateElementSelection` пересчитывает по живым rect'ам, и всё, что вышло из рамки, снимается). Прокрутишь достаточно далеко во время драга — ранее задетые элементы выпадут. Ровно то же самое в демках самого viselect, обходить это мы не стали.

Если понадобится «липкое» выделение — накапливай его сам, вместо того чтобы зеркалить store:
