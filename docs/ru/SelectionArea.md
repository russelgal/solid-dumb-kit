...
- **Игнорируемые цели** — жест, начатый на `button, a, input, select, textarea, [data-drag-handle], [data-no-select]`, не стартует, поэтому элементы управления и ручки перетаскивания продолжают работать. Повесь `data-no-select`, чтобы исключить свой элемент.
- **Автоскролл** — протяжка у края контейнера прокручивает его, ускоряясь по мере ухода за край.
- **Рамка не вылезает наружу** — она прижата к границам контейнера. Это не только про вид: рамка — абсолютно спозиционированный потомок, и выехав за пределы она растянула бы контейнеру `scrollWidth`/`scrollHeight`, породив полосы прокрутки прямо во время выделения.
