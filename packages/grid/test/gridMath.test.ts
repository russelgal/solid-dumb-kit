...
describe('packFlow — ужимание по minW', () => {
  it('блок ужимается до остатка строки вместо переезда вниз', () => {
    // b хочет 8 колонок, свободны 6 — с minW=4 он встаёт рядом, а не под a
    const out = packFlow([span('a', 6), { ...span('b', 8), minW: 4 }], 12)
    expect(out.map((p) => [p.col, p.row, p.w])).toEqual([[0, 0, 6], [6, 0, 6]])
  })

  it('ужимается ровно до остатка, а не до минимума', () => {
    const out = packFlow([span('a', 9), { ...span('b', 8), minW: 1 }], 12)
    expect(out[1].w).toBe(3)          // 3 свободных — их и занял
  })

  it('когда остаток меньше минимума, уезжает вниз целым', () => {
    const out = packFlow([span('a', 10), { ...span('b', 8), minW: 4 }], 12)
    expect([out[1].col, out[1].row, out[1].w]).toEqual([0, 1, 8])
  })

  it('на просторном месте берёт желаемую ширину, а не минимальную', () => {
    const out = packFlow([{ ...span('a', 8), minW: 2 }], 12)
    expect(out[0].w).toBe(8)
  })

  it('без minW ведёт себя как раньше — переносит вниз целиком', () => {
    const out = packFlow([span('a', 6), span('b', 8)], 12)
    expect([out[1].col, out[1].row, out[1].w]).toEqual([0, 1, 8])
  })

  it('minW шире желаемой ширины не растягивает блок', () => {
    const out = packFlow([{ ...span('a', 3), minW: 9 }], 12)
    expect(out[0].w).toBe(3)
  })

  it('ужимание учитывает высоту: под высоким соседом место уже', () => {
    // a занимает колонки 0–5 в двух строках; b встаёт правее ужатым,
    // c высотой 1 идёт следом
    const out = packFlow([span('a', 6, 2), { ...span('b', 9, 2), minW: 3 }], 12)
    expect([out[1].col, out[1].row, out[1].w]).toEqual([6, 0, 6])
  })
})
